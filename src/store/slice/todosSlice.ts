...
export const todosPageSlice = createSlice( {
    ...
    extraReducers: ( builder ) => {
        builder
          ...
          .addCase( createTodo.pending, ( state: TodosSchema ) => {
              state.isLoading = true;
              state.error = null;
          } )
          .addCase( createTodo.fulfilled, ( state: TodosSchema, action: PayloadAction<ITodo> ) => {
              state.isLoading = false;
              state.todosDate = [...state.todosDate, action.payload]
          } )
          .addCase( createTodo.rejected, ( state: TodosSchema, action ) => {
              state.isLoading = false;
              state.error = action.payload || null;
          } )
